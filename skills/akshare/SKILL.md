...
- China-market findata-toolkit-cn 中的公开数据能力，如 CPI/PPI、PMI、社融、M2、LPR、Shibor、北向资金和公开行情，可作为 AkShare 补充方向说明。

## 限制

- SATS 若未安装或未实现 `akshare_provider`，只能建议后续接入，不可声称数据已获取。
- AkShare 接口来源分散，字段和频率可能变化；回答时说明数据口径需要校验。
- 不迁入或执行 `finskills` 的 `scripts/*.py`；所有取数必须走 SATS provider 适配层。
