...
| 原生个股深研 | `deep-analysis`, Agent 工具 `research.deep_stock_analysis` | Markdown / JSON artifact | A 股普通股票核心深研闭环：维度评分、投资人面板和综合摘要 |
| Serenity AI 卡位筛选 | `serenity-screen`, Agent 工具 `research.serenity_screen` | 终端 Markdown / JSON artifact / DuckDB | AI/科技供应链候选池、证据增强、卡位评分和风险罚分 |
...
- 默认行为：终端直接显示 Markdown 研究报告；未加 `--noreport` 时同时写入 Markdown / JSON artifact。
...
- 默认行为：终端显示完整 Markdown 排名报告；评估结果写入 `screening_results`，规则名为 `serenity_bottleneck`。
...
自然语言 Agent 在显式提到 Serenity、卡位、卡脖子、稀缺层或供应链瓶颈时自动调用；AI、半导体、光通信、先进封装、算力电力散热或机器人主题与“选股/筛选/推荐”组合出现时也会自动调用。普通短线推荐继续使用 `research.discover_opportunities`，显式指定其他筛选规则时其他规则优先。
...
- `--plan-only` 只生成目标、边界、步骤、分析模式和验证项，不执行工具。
...
计划 JSON 顶层字段为 `schema_version`、`name`、`start_date`、`end_date`、`active_windows` 和 `items`。每只股票包含一个或多个 `trigger_groups`；组内条件全部满足才触发，不同组独立触发。首版条件支持：

- 对象：当前股票，或上证指数、深证成指、创业板指、沪深300。
- 指标：`latest_price`、`change_points`、`pct_change`。
- 运算符：`>=`、`>`、`<=`、`<`。
- 动作：`notify`、`buy`、`sell`。
