"""SATS factor zoo base operators.
...
class Alpha:
    """Lightweight handle for a registered factor (registry-owned)."""
...
class AlphaCompute(Protocol):
    """Structural protocol every factor ``.py`` module satisfies."""

    def __call__(self, panel: dict[str, pd.DataFrame]) -> pd.DataFrame: ...


Factor = Alpha
FactorCompute = AlphaCompute
