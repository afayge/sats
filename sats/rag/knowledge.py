...
DEFAULT_COLLECTIONS = {
    ...
    "price-action": {
        "name": "price-action",
        "description": "阴线买入、开盘溢价率、量价关系、均线信号、520均线、RSI极值、趋势执行纪律、左右倍量、主力洗盘形态、量价支撑、仓位止损和短线去留纪律知识库。",
        "paths": ("knowledge/price_action",),
        "tags": ("price-action", "短线纪律", "风控"),
    },
    "signals": {
        "name": "signals",
...
def _query_terms(query: str) -> list[str]:
    ...
    tokens.extend(phrase for phrase in domain_phrases if phrase in text)
