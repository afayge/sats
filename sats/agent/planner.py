...
def build_agent_plan(
    ...
    *,
    ...
    if llm_factory is not None:
        payload = _llm_plan_payload(message, settings=settings, policy=policy, llm_factory=llm_factory, tool_registry=tool_registry)
        plan = _plan_from_payload(payload, message=policy_message or message, tool_registry=tool_registry)
        if plan.steps:
            return _augment_plan(plan, message=policy_message or message)
    return _augment_plan(_fallback_plan(policy_message or message, policy=policy, tool_registry=tool_registry), message=policy_message or message)
...
def _llm_plan_payload(
    ...
    *,
    ...
    try:
        ...
        llm = llm_factory()
    try:
        response = llm.chat(_planner_messages(message, policy=policy, tool_registry=tool_registry), timeout=getattr(settings, "llm_timeout_seconds", None))
    except TypeError:
        response = llm.chat(_planner_messages(message, policy=policy, tool_registry=tool_registry))
    except Exception:
        return {}
...
def _planner_messages(message: str, *, policy: AgentExecutionPolicy, tool_registry: AgentToolRegistry | None) -> list[dict[str, str]]:
...
def _fallback_plan(message: str, *, policy: AgentExecutionPolicy, tool_registry: AgentToolRegistry | None = None) -> AgentPlan:
    text = str(message or "").strip()
    lowered = text.lower()
    steps: list[AgentStep] = []
    symbols = extract_stock_symbols(text)
    clean_symbols = symbols or ["000001"] if _needs_symbol_default(text) else symbols
    if _is_trade_request(text):
        ...
        steps.append(
            _tool_step(
                ...
                "data.index_daily",
                "获取指数日 K",
                {"index_codes": indices, "start_date": _date_days_before(end_date, 14), "end_date": end_date},
                side_effect="write_db",
            )
        )
        ...
        trade_date = _analysis_trade_date(text)
        steps.append(
            _tool_step(
                "stock_daily",
                "data.stock_daily",
                "获取个股日 K",
                {"symbols": clean_symbols, "start_date": _date_days_before(trade_date, 180), "end_date": trade_date},
                side_effect="write_db",
            )
        )
        steps.append(_tool_step("realtime_quote", "data.realtime_quotes", "获取实时报价", {"symbols": clean_symbols}, side_effect="write_db"))
    ...
    elif any(term in text for term in ("筛选", "上涨", "机会", "推荐", "候选")):
...
def _augment_plan(plan: AgentPlan, *, message: str) -> AgentPlan:
    ...
    symbols = _plan_stock_symbols(steps) or extract_stock_symbols(text)
    if symbols and _is_stock_analysis_request(text) and not _is_market_analysis_request(text):
        ...
        if not _has_tool(steps, "research.stock_context"):
            ...
            steps.insert(insert_at, _tool_step("stock_context", "research.stock_context", "获取个股上下文", {"symbols": symbols, "trade_date": _analysis_trade_date(text)}, side_effect="readonly"))
        if not _has_internal_analysis_kind(steps, "indicators"):
            ...
            steps.insert(
                ...
                _tool_step(
                    ...
                    "research.internal_analysis",
                    "补充技术信号",
                    {"kind": "indicators", "symbols": symbols, "trade_date": _analysis_trade_date(text), "horizons": _market_horizons(text)},
                    side_effect="readonly",
                ),
            )
        if not _has_internal_analysis_kind(steps, "analyze_signals"):
            insert_at = _insert_before_factor_or_final(steps)
            steps.insert(insert_at, _analyze_signals_step(symbols, _analysis_trade_date(text), text))
        if not _has_internal_analysis_kind(steps, "factor_summary") and not _has_tool(steps, "factor.pick"):
            insert_at = _final_index(steps)
            steps.insert(
                insert_at,
                _tool_step(
                    ...
                    "research.internal_analysis",
                    "补充因子画像",
                    {"kind": "factor_summary", "symbols": symbols, "trade_date": _analysis_trade_date(text)},
                    side_effect="readonly",
                ),
            )
    return AgentPlan(
...
def _analysis_trade_date(text: str) -> str:
    try:
        return extract_trade_date(text) or agent_today()
    except ValueError:
        return agent_today()
