...
class ConversationRuntimeTest(unittest.TestCase):
        ...
        self.assertEqual(
            ...
            [
                (
                    ...
                    {"horizons": ["next_week"], "dimensions": ["core_indices", "market_breadth"], "trade_date": "20260626"},
                )
            ],
        )
    ...
    def test_conversation_injects_market_relative_time_arguments(self) -> None:
        ...
        cases = (
            ("评价今天的大盘走势，以及热点板块", {"trade_date": "20260626"}),
            ("评价昨天的大盘走势", {"trade_date": "20260625"}),
            ("预测明天大盘走势", {"horizons": ["tomorrow"]}),
            ("预测未来几日大盘走势", {"horizons": ["tomorrow", "day_after_tomorrow"]}),
            ("预测未来3天大盘走势", {"horizons": ["tomorrow", "day_after_tomorrow"]}),
            ("分析今天大盘走势，预测明天走势", {"trade_date": "20260626", "horizons": ["tomorrow"]}),
            ("过去几天大盘和热点板块", {"trade_date": "20260626"}),
            ("过去3天大盘和热点板块", {"trade_date": "20260626"}),
            ("2026-06-25 不是昨天的大盘走势", {"trade_date": "20260625"}),
            ("评价大盘走势和热点板块", {"horizons": ["today"]}),
        )
        settings = _settings(self)
    ...
    def test_conversation_normalizes_model_relative_trade_date_literal(self) -> None:
        ...
        expected_arguments = {"trade_date": "20260630", "horizons": ["today"], "dimensions": ["core_indices"]}
        self.assertFalse(result.requires_clarification)
        self.assertEqual(registry.executed, [("research.market_context", expected_arguments)])
        tool_observation = next(item for item in result.observations if item.kind == "tool")
        self.assertEqual(tool_observation.payload["arguments"], expected_arguments)
